...
@unittest.skipUnless(is_cv2_available(), "Learned descriptor tests require cv2")
class Task3LearnedDescriptorTests(unittest.TestCase):
    def test_learned_descriptor_mode_produces_verifiable_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            settings = MvpRuntimeSettings(
                ...
                task3_learned_pretrained=False,
                task3_learned_min_similarity=0.5,
            )
            ...
            matches = matcher.match(frame, frame_bytes, decoded_frame=decoded, mode="learned_descriptor")
            verified = verify_matches(frame, matches, decoded_frame=decoded, min_inliers=2, min_similarity=0.5, min_corroboration=0.0)

            self.assertLessEqual(len(matches), 1)
            if matches:
                self.assertEqual(matches[0].metadata["matcher_source"], "task3_learned_descriptor")
